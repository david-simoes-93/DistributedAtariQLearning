...
class Worker:
    ...
    def take_action_from_network(self, sess, network, previous_screen, action_indexes):
        action, value = sess.run([network.best_action_index, network.value],
                                              feed_dict={network.inputs: [previous_screen]})
        return action, np.max(value)
        ...
        while not coord.should_stop() and global_step_count < max_steps:
            ...
            while episode_step_count < max_episode_length:
                ...
                if np.random.random() < exploration_rate:
                    ...
                    action, value = \
                        self.take_action_from_network(sess, self.local_AC.network, previous_screen, action_indexes)
                ...
                if not terminal and len(episode_buffer) == self.batch_size and episode_step_count < max_episode_length - 1:
            ...
            if episode_count % self.summary_save_episodes == 0 and episode_count != 0:
                ...
                if self.is_chief and episode_count % 50 == 0:
                    debug_states = [np.zeros(40) for _ in range(40)]
                    for i in range(40):
                        debug_states[i][i]=1
                    buff = sess.run(self.local_AC.target_network.best_q,
                                          feed_dict={self.local_AC.target_network.inputs: debug_states})
                    mpl.plot(buff)
                    #print(["% 4.2f" % x for x in buff])
