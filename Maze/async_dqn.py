...
#   tensorboard --logdir=QBatch:'./DistributedAtariQLearning/summaries0',NStep:'./DistributedAtariQLearningNstep/summaries1',1steo:'./DistributedAtariQLearning1step/summaries2' --port 7000

SEED = 123

import os

from Maze.Bridge import GymBridge
...
flags.DEFINE_integer('anneal_epsilon_timesteps', 450000*amount_of_session_runs_per_step, 'Number of timesteps to anneal epsilon.')
...
flags.DEFINE_boolean('nstep', False, 'If true, run n-step')
...
def actor_learner_thread(thread_id, env_g, test_env, session, graph_ops, num_actions, summary_ops,
    ...
    if test_env is not None:
        with open("thread_started_" + str(thread_id), "w+") as log_file:
            while not session.should_stop():
                counter = 0
                ep_rew = 0
                s_t = test_env.reset()
                term = False
                while counter < 1000 and not term:
                    if random.random() <= 0.01:
                        action_index = random.randrange(num_actions)
                    else:
                        q_values_, global_step = session.run([q_values, increment_global_step],
                                                                          feed_dict={s: [s_t]})
                        action_index = np.argmax(q_values_[0])

                    s_t, rew, term, _ = test_env.step(action_index)
                    counter += 1
                    ep_rew += rew
                #summary_str, _ = session.run([summary_op, update_ops], feed_dict={summary_placeholders[0]: 0,
                #                                                                  summary_placeholders[1]: 0,
                #                                                                  summary_placeholders[2]: 0.01,
                #                                                                  summary_placeholders[3]: 0,
                #                                                                  summary_placeholders[4]: ep_rew})
                #writer.add_summary(summary_str, global_step)
                log_file.write(str(global_step) + " " + str(ep_rew) + "\n")
                time.sleep(30)
    ...
    with open("thread_started_"+str(thread_id), "w+"):
        pass
    while True:
        time.sleep(1)
        all_started = True
        for i in range(int(FLAGS.slaves_per_url)):
            all_started = all_started and os.path.isfile("thread_started_"+str(i))
        if all_started:
            break
    print("Started")
    ...
    while not session.should_stop():  # < TMAX:
        ...
        while True:
            ...
            if epsilon > final_epsilon:
                epsilon = np.max([1-(1-final_epsilon)*(global_step/FLAGS.anneal_epsilon_timesteps), final_epsilon])
            ...
            if t % FLAGS.network_update_frequency == 0 or terminal:
                if s_batch:
                    ...
                    for index in range(len(y_batch) - 2, -1, -1):
                        ...
                        elif FLAGS.nstep:  # n-step
                            discounted_rewards[index] = discounted_rewards[index + 1] * FLAGS.gamma + r_batch[index]
                        else:  # 1-step
                            discounted_rewards[index] = y_batch[index]
                    _, loss_val, global_step = session.run([grad_update, loss, global_step_var], feed_dict={y: discounted_rewards,
                                                        a: a_batch,
                                                        s: s_batch})
                    loss_stats.append(loss_val)
            ...
            if terminal:
                summary_str, _ = session.run([summary_op, update_ops], feed_dict={summary_placeholders[0]: ep_reward,
                                                   ...
                                                   summary_placeholders[3]: np.mean(loss_stats),
                                                   summary_placeholders[4]: 0})
                writer.add_summary(summary_str, global_step)
                ...
                print("THREAD:", thread_id, "/ TIME", T, "/ TIMESTEP", t, global_step, "/ EPSILON", epsilon, "/ REWARD", ep_reward,
                      ...
                      np.min([global_step / float(FLAGS.anneal_epsilon_timesteps),1]))
                break
...
def setup_summaries():
    ...
    logged_loss = tf.Variable(0.)
    tf.summary.scalar("Loss", logged_loss)
    logged_time_taken_eval = tf.Variable(0.)
    tf.summary.scalar("Time Taken", logged_time_taken_eval)
    logged_T = tf.Variable(0.)
    summary_vars = [episode_reward, episode_ave_max_q, logged_epsilon, logged_loss, logged_time_taken_eval]
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.thread_index,
            ...
            with tf.train.MonitoredTrainingSession(master=server.target, is_chief=is_chief,
                ...
                # Start num_concurrent actor-learner training threads
                actor_learner_thread(FLAGS.thread_index, env, GymBridge() if is_chief else None,
                                     session, graph_ops,
